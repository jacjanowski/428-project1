...
//test commit
public class ClasscubeWeather : MonoBehaviour
{
    public GameObject weatherTextObject;
    // add your personal API key after APPID= and before &units=
    string url = "https://api.openweathermap.org/data/2.5/weather?q=Lubbock,usa&APPID=344db1c9a6a32d8f039c400527f61de6&units=imperial";
    ...
    IEnumerator GetRequest(string uri)
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(uri))
        {
            ...
            else
            {
                ...
                double celsius = Math.Floor((easyTempF - 32) * (.5556));

                weatherTextObject.GetComponent<TextMeshPro>().text = "" + celsius.ToString() + "°C\n" + conditions;
            }
        }
    }
}
